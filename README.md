...
부트캠프 조직 레포지토리이며 contributors의 나머지 두 계정은 리뷰를 담당하신 강사님 계정입니다.
...
금속과 보석에 각각 PBR 재질을 입힌 뒤 glTF(.glb)로 내보냈습니다. 톤매핑은 ACES와 Khronos PBR Neutral 두 가지를 렌더링 결과로 직접 비교했습니다. ACES 적용 시 스톤과 폴리시드 메탈의 대비가 눈에 띄게 약해지는 반면, Neutral은 그 대비를 더 선명하게 유지했습니다. 주얼리다운 광택이 중요한 장면이었기 때문에 시각적 결과를 기준으로 Neutral을 최종 선택했습니다.
...
맨 위 GIF에서 보이는 움직임이 이 부분입니다. 스크롤 진행도를 `requestAnimationFrame` 루프에서 부드럽게 보간(lerp)해, 반지가 화면을 가로지르며 이동·회전(Z roll·pitch)하고 텍스트 레이아웃과 맞물리도록 만들었습니다. 여기에 마우스 움직임 기반 패럴랙스 틸트를 스크롤 모션과 분리된 레이어로 얹어, 스크롤과는 별개로 화면 위치 기준으로 자연스럽게 반응하도록 함과 동시에 결합도를 낮추려 했습니다.
...
15개 페이지를 작업하면서 매번 색상값을 직접 고르면 미묘한 불일치가 생길 것 같았습니다. 그래서 색상을 값이 아닌 역할 이름으로 관리하는 방향을 택했습니다.
