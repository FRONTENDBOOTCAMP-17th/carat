...
가상의 주얼리 브랜드 PRISME를 주제로 제작한 프론트엔드 파이널 프로젝트입니다. 기획, 디자인, 프론트엔드 개발, 3D 에셋 제작까지 1인으로 진행했습니다.
...
PRISME은 에디토리얼 매거진처럼 브랜드 경험 자체를 전달하는 방향으로 기획했습니다.
...
- **[스크린샷](#스크린샷)**
- **[프로젝트 목표](#프로젝트-목표)**
- **[3D 히어로: 직접 제작한 에셋과 인터랙션](#3d-히어로-직접-제작한-에셋과-인터랙션)**
  - [모델링](#모델링) · [텍스처링 · 재질](#텍스처링--재질) · [씬 통합](#씬-통합) · [스크롤 애니메이팅](#스크롤-애니메이팅)
- **[설계 과정에서 중요하게 다룬 부분](#설계-과정에서-중요하게-다룬-부분)**
  - [레이아웃 구조](#레이아웃-구조) · [반응형](#반응형) · [3D 씬과 로딩 화면 동기화](#3d-씬과-로딩-화면-동기화) · [모달의 포커스 관리](#모달의-포커스-관리) · [디자인 토큰 시스템](#디자인-토큰-시스템) · [퍼지 검색](#퍼지-검색) · [커스텀 커서](#커스텀-커서)
- **[접근성](#접근성)**
- **[화면 구조](#화면-구조)**
- **[유저 플로우](#유저-플로우)**
- **[UX 분기](#ux-분기)**
  - [/search](#search) · [/products/:id](#productsid) · [/essential](#essential) · [/wishlist](#wishlist) · [/contact](#contact)
- **[전역 상태](#전역-상태)**
  - [테마](#테마-dark--light) · [언어](#언어-ko--en) · [인증](#인증)
- **[기술 스택](#기술-스택)**
- **[실행](#실행)**
- **[이미지 출처](#이미지-출처)**
- **[라이선스](#라이선스)**
...
그래서 일반적인 쇼핑몰 구조 대신 스크롤 중심의 에디토리얼 레이아웃, 3D 히어로를 활용한 브랜드 인트로, 컬렉션별 스토리텔링, 절제된 인터랙션을 통해 사용자가 브랜드를 탐색하는 과정 자체를 설계하는 데 집중했습니다.
...
거들(girdle)을 감싸는 메탈 밴드, 밴드에 물린 마퀴즈(marquise) 스톤 3개, 밴드 양쪽을 장식하는 파베(pavé) 스톤 구조를 Blender로 직접 모델링하고, 카메라를 정면으로 마주 보도록 기본 자세를 잡았습니다.
...
맨 위 GIF에서 보이는 움직임이 이 부분입니다. 스크롤 진행도를 `requestAnimationFrame` 루프에서 부드럽게 보간(lerp)해, 반지가 화면을 가로지르며 이동·회전(Z roll·pitch)하고 텍스트 레이아웃과 맞물리도록 만들었습니다. 여기에 마우스 움직임 기반 패럴랙스 틸트를 스크롤 모션과 분리된 레이어로 얹어, 화면 기준으로 자연스럽게 반응하도록 했습니다.
...
뷰포트가 최대 너비(1440px)를 넘어서면 `mx-auto`로 인해 컨테이너가 가운데 정렬이 되면서 시각적으로 여백이 생기므로 데스크탑엔 별도 패딩이 필요 없다고 판단했습니다. 다만 화면이 1440px보다 좁아지기 시작하는 구간부터는 콘텐츠 영역이 전체 화면을 가득 채우게 되는데, 여기서 패딩이 없다면 콘텐츠가 화면 가장자리에 그대로 붙어버립니다. 그래서 해당 브레이크포인트 이하부터는 양 옆에 패딩을 두었습니다.
...
초기 진입 시 3D 씬이 준비되기 전에 검은 화면이 노출되거나 오브젝트가 갑자기 나타나는 문제가 있었습니다.
...
사용자가 정확한 상품명을 외우고 있다고 가정하는 검색은 현실적이지 않다고 생각했습니다. "은반지", "14k" 같은 단편적인 단어로도, 오타가 섞여도 관련 결과가 나오는 것이 더 자연스러운 탐색 경험이라고 생각했습니다.
