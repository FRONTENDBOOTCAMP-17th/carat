...
이 3단 구조와 패딩 값은 피그마 그리드 시스템으로 먼저 설계했습니다. 뷰포트 프레임(전체 너비) 안에 배경을 담당하는 섹션 프레임을 두고, 그 안에 콘텐츠 프레임을 Fill·Hug·최소 너비 제약으로 배치하는 구조로 잡았고, 그리드는 모바일 마진 16·거터 16, 데스크탑은 1440px 안에서 거터 24px인 12컬럼을 기준으로 삼았습니다.

이걸 반응형 패딩으로 옮기면서 판단이 하나 필요했습니다. 뷰포트가 최대 너비(1440px)를 넘어서면 `mx-auto`가 자동으로 여백을 만들어주므로 데스크탑엔 별도 패딩이 필요 없다고 판단했습니다. 다만 1024~1440px 구간은 아직 뷰포트가 최대 너비보다 좁아 센터링이 작동하지 않는데, 여기서 패딩까지 0이면 콘텐츠가 화면 가장자리에 그대로 붙어버립니다. 그래서 이 구간에만 32px(`lg:px-8`) 패딩을 따로 두어 여백을 채웠습니다.
