...
3D 섹션의 비주얼 작업 단계에서는 스크롤을 내릴 때 반지가 어떻게 회전해야 하는지, 어떻게 라이팅 처리를 해야 시각적으로 임팩트가 있어 보일지에 대해 고민했습니다.

### 모델링

거들(girdle)을 감싸는 메탈 밴드, 밴드에 물린 마퀴즈(marquise) 스톤 3개, 밴드 양쪽을 장식하는 파베(pavé) 스톤 구조를 Blender로 직접 모델링하고, 카메라에 역동적인 구도로 잡히면서 마퀴즈 스톤의 반짝임이 잘 보이도록 기본 자세를 잡았습니다.

### 텍스처링 · 재질

금속과 보석에 각각 PBR 재질을 입힌 뒤 glTF(`.glb`)로 내보냈습니다. 톤매핑은 ACES 대신 **Khronos PBR Neutral**을 선택했는데, ACES를 사용했을 땐 스톤과 메탈 특유의 강한 대비가 시각적으로 약해져 임팩트가 사라졌기 때문입니다. Khronos로 그 대비를 살려 주얼리다운 광택을 유지했습니다.
...
섹션별로 역할이 크게 달라지는 사이트 특성상, 시각 레이어와 콘텐츠 레이어를 분리하는 구조가 필요했습니다.
